...
const Home = () => {
  ...
  const handleSearch = async () => {
    ...
    if (query) {
      try {
        const response = await fetch(
          `https://vhxmbqg0-8080.asse.devtunnels.ms/api/books/search?query=${encodeURIComponent(
            query
          )}`
        );
        if (!response.ok) {
    ...
    <div className="min-h-screen bg-gray-100">
      <nav className="sticky mx-auto flex top-0 z-10 bg-gray-100 shadow-md justify-between items-center">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <Link
              to="https://algaray02.github.io/web-perpus-pcc-class/"
      ...
      <main className="container mx-auto px-0 max-w-full ">
        <section
          ...
          className="grid md:grid-cols-2 gap-8 items-center py-12 px-4 pt-36 md:pt-20 pb-96 md:pb-40 opacity-100 transition-opacity duration-500"
        >
              ...
              {selectedBook ? (
                <div className="flex gap-6">
                  <div className="w-48 h-72 rounded-lg overflow-hidden bg-gray-100">
                    <img
                      src={`https://vhxmbqg0-8080.asse.devtunnels.ms/images/books/${selectedBook.imgUrl}`}
